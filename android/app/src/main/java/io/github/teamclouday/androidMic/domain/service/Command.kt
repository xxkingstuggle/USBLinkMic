...
private const val ID_WAVE_LEVELS: String = "ID_WAVE_LEVELS"
private const val ID_CONTROLLED_BY_MAC: String = "ID_CONTROLLED_BY_MAC"
private const val ID_ACTIVE_MODE: String = "ID_ACTIVE_MODE"
private const val ID_ACTIVE_SAMPLE_RATE: String = "ID_ACTIVE_SAMPLE_RATE"
private const val ID_ACTIVE_CHANNEL_COUNT: String = "ID_ACTIVE_CHANNEL_COUNT"
private const val ID_ACTIVE_AUDIO_FORMAT: String = "ID_ACTIVE_AUDIO_FORMAT"
    ...
    companion object {
        ...
        suspend fun fromPref(
            ...
            when (mode) {
                ...
                Mode.ADB -> {
                    val p = port.ifEmpty { DEFAULT_PORT.toString() }
                    if (!checkPort(p)) {
...
data class ResponseData(
    ...
    val waveLevels: FloatArray? = null,
    val kind: ResponseKind = ResponseKind.Standard,
    val controlledByMac: Boolean? = null,
    val activeMode: Mode? = null,
    val activeSampleRate: SampleRates? = null,
    val activeChannelCount: ChannelCount? = null,
    val activeAudioFormat: AudioFormat? = null,
) {


    companion object {
        fun fromMessage(msg: Message): ResponseData {
            val kindIndex = msg.what
            return ResponseData(
                kind = ResponseKind.entries.getOrElse(kindIndex) { ResponseKind.Standard },
                ...
                waveLevels = msg.data.getFloatArray(ID_WAVE_LEVELS),
                msg = msg.data.getString(ID_MSG),
                controlledByMac = msg.data.getBoolean(ID_CONTROLLED_BY_MAC, false),
                activeMode = msg.data.getOrdinal(ID_ACTIVE_MODE)?.let { Mode.entries[it] },
                activeSampleRate = msg.data.getOrdinal(ID_ACTIVE_SAMPLE_RATE)
                    ?.let { SampleRates.entries[it] },
                activeChannelCount = msg.data.getOrdinal(ID_ACTIVE_CHANNEL_COUNT)
                    ?.let { ChannelCount.entries[it] },
                activeAudioFormat = msg.data.getOrdinal(ID_ACTIVE_AUDIO_FORMAT)
                    ?.let { AudioFormat.entries[it] },
            )
        }
    }
    ...
    fun toResponseMsg(): Message {
        ...
        this.waveLevels?.let { r.putFloatArray(ID_WAVE_LEVELS, it) }
        this.controlledByMac?.let { r.putBoolean(ID_CONTROLLED_BY_MAC, it) }
        this.activeMode?.let { r.putInt(ID_ACTIVE_MODE, it.ordinal) }
        this.activeSampleRate?.let { r.putInt(ID_ACTIVE_SAMPLE_RATE, it.ordinal) }
        this.activeChannelCount?.let { r.putInt(ID_ACTIVE_CHANNEL_COUNT, it.ordinal) }
        this.activeAudioFormat?.let { r.putInt(ID_ACTIVE_AUDIO_FORMAT, it.ordinal) }
