...
public class LinkNetService extends VpnService {
    ...
    private static final String TAG = LinkNetService.class.getSimpleName();
    ...
    public static void start(Context context, VpnConfiguration config) {
        Intent intent = new Intent(context, LinkNetService.class);
        intent.setAction(ACTION_START_VPN);
        intent.putExtra(LinkNetService.EXTRA_VPN_CONFIGURATION, config);
    ...
    private static final class RelayTunnelConnectionStateHandler extends Handler {
        ...
        private final LinkNetService vpnService;

        private RelayTunnelConnectionStateHandler(LinkNetService vpnService) {
